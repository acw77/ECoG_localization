function error = dbs_dicm2nifti (options)
%
%
...
%
% Finish
% See: https://users.fmrib.ox.ac.uk/~robson/internal/Dicom2Nifti.htm
% Download: http://www.fmrib.ox.ac.uk/~robson/internal/Dicom2Nifti111.m
...
filenames_list = filenames_list(~ismember({filenames_list.name},{'.' '..' '.DS_Store'}));
...
for counter = 1 : size(filenames_list,1)
    ...
    occurences_of_dcm = strfind(filenames_list(counter).name , '.dcm');
    ...
    if length(occurences_of_dot)==0  | length(occurences_of_dcm)~=0 | length(occurences_of_IMA)~=0 | length(occurences_of_ima)~=0 && (exist(file_directory,'file')~=0 && exist(file_directory,'dir')==0) %only consider files that have no '.' in the  
